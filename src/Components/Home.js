    ...
    ***********************************************/
    ...
    React.useEffect(() => {
        if (projectsData.length > 0) {
            const optimized = optimizeProjectsGrid(projectsData, expandedProject);
            setGridConfig(optimized);
        }
            ...
            <div className="projectsscreen">
                <div className="maxwidthcenterwrapper">
                    <div className="projects-header">
                        <h2 className="projects-title whitetext text350 bold900 spacedLetters005 lineheight70">Projects</h2>
                    </div>
                    <div className="projects-grid">
                        {gridConfig.map((project, i) => (
                            <ProjectCard 
                                key={project.id || i} 
                                data={project} 
                                index={i}
...
function ProjectCard(props) {
    let { data, index, expandedProject, setExpandedProject } = props
    const [isHovered, setIsHovered] = useState(false)
    const [hoverStartTime, setHoverStartTime] = useState(null)
    const [expansionTimer, setExpansionTimer] = useState(null)
    
    // Determine if link is GitHub
    const isGitHub = data.link && data.link.includes('github.com')
    
    // Handle hover timing for expansion
    const handleMouseEnter = () => {
        setIsHovered(true)
        setHoverStartTime(Date.now())
        
        // Set 3-second timer for expansion
        const timer = setTimeout(() => {
            if (expandedProject !== data.id) {
    ...
    // Check if image should be on left (for wide cards) or top (for square cards)
    const isWideCard = data.width > data.height
    
    return(
        <div 
            className={`project-card project-card-${data.width}x${data.height} ${isHovered ? 'hovered' : ''}`}
            data-index={index}
        ...
        >
            <div className="project-card-inner">
                {data.image && (
                    <div className={`project-image ${isWideCard ? 'project-image-left' : 'project-image-top'}`}>
